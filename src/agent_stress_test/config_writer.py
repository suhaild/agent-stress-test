...
spec's YAML file, in place, via block-splices that preserve every comment and
unrelated line (a plain ``yaml.safe_load``/``dump`` round-trip would not).
"""
...
def _replace_system_prompt_block(raw_yaml: str, new_system_prompt: str) -> str:
    """Splice a new value into the `system_prompt: |` block; every other
    line is untouched, so PyYAML re-serializing the whole file (and dropping
    every comment) is avoided."""
    ...
    # Trailing blank lines belong to formatting between keys, not the scalar.
...
def apply_system_prompt(path: str | Path, new_system_prompt: str) -> AgentSpec:
    ...
    Validates by reloading before returning; on failure, restores the
    original file content so a bad edit never leaves the file broken.
    """
    path = Path(path)
...
def _append_rule_block(raw_yaml: str, rule: Rule) -> str:
    ...
    list; the list-shaped analog of `_replace_system_prompt_block`."""
    lines = raw_yaml.splitlines()
    ...
    # Trailing blank lines belong to formatting between keys, not the list.
    while end_idx > start_idx + 1 and not lines[end_idx - 1].strip():
        end_idx -= 1
...
def apply_candidate_rule(path: str | Path, rule: Rule) -> AgentSpec:
    ...
    Refuses a duplicate `id` — the judge, verdicts, and regression cases all
    look rules up by id. Validates and rolls back on failure, same as
    `apply_system_prompt`.
    """
    path = Path(path)
